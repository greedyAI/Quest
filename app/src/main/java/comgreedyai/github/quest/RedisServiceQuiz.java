...
class RedisServiceQuiz {
    ...
    interface Service {
        ...
        @GET("GET/{key}")
        Call<GetResponse> getQuiz(@Path("key") String key);

        // Delete the key and its value
        @GET("DEL/{key}")
        Call<DelResponse> deleteQuiz(@Path("key") String key);

        // Store a value (in a Java class ListItem) under a key
        @PUT("SET/{key}")
        Call<SetResponse> addQuiz(@Path("key") String key, @Body Quiz body);
    ...
    class GetResponse {
        @SerializedName("GET")
        Quiz item; // ListItem is a Java class that holds data in your RecyclerView
    }
