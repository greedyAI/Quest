...
class RedisServiceLogin {
    ...
    interface Service {
        ...
        @GET("GET/{key}")
        Call<GetResponse> getPass(@Path("key") String key);

        // Delete the key and its value
        @GET("DEL/{key}")
        Call<DelResponse> deleteUser(@Path("key") String key);

        // Store a value (in a Java class ListItem) under a key
        @PUT("SET/{key}")
        Call<SetResponse> addUser(@Path("key") String key, @Body String body);
